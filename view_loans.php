<?php
session_start();
$page_title = "Current Rentals | Library Rental System";
...
<div class="container-fluid">
    <div class="row">
        ...
        <div class="col-sm-10">
            <h1 class="pt-5 pb-4 text-center">Current rentals</h1>
                ...
                <table class="table table-hover" id="myTable">
                    ...
                    <tbody>
                        <?php foreach ($rentals as $rental): ?>
                            <?php $overdue = $rental['due_back'] < $today; ?>
                            <tr class="<?= $overdue ? 'table-danger' : '' ?>">
                                <td><?= htmlspecialchars($rental['item_name']) ?></td>
                                <td><?= htmlspecialchars($rental['category']) ?></td>
                                <td><?= htmlspecialchars($rental['borrower_name']) ?></td>
                                <td><?= htmlspecialchars($rental['due_back']) ?></td>
                                <td>
                                    <?php if ($overdue): ?>
